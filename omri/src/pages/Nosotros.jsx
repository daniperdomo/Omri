import React from 'react';

const Nosotros = () => {
  return (
    <div className="bg-gray-100">
      {/* Sección: Sobre nosotros */}
      <div className="relative h-96">
        {/* Imagen de fondo */}
        <img
          src="/images/nosotros-background.jpg" // Reemplaza con la ruta de tu imagen
          alt="Fondo sobre nosotros"
          className="w-full h-full object-cover"
        />
        {/* Cuadro de texto sobrepuesto */}
        <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="max-w-2xl text-center p-6 bg-white bg-opacity-90 rounded-lg shadow-lg">
            <h1 className="text-4xl font-bold text-gray-800 mb-4">Sobre nosotros</h1>
            <p className="text-lg text-gray-600">
              En [Nombre de la Empresa], nos dedicamos a ofrecer soluciones innovadoras y de alta
              calidad para nuestros clientes. Desde nuestro inicio en [año], hemos trabajado
              incansablemente para convertirnos en líderes en [industria o sector]. Nuestro equipo
              está comprometido con la excelencia y la satisfacción del cliente.
            </p>
          </div>
        </div>
      </div>

      {/* Sección: Ubicación */}
      <div className="max-w-7xl mx-auto py-12 px-4">
        <h2 className="text-3xl font-bold text-gray-800 mb-6 text-center">Ubicación</h2>
        <div className="flex flex-col md:flex-row gap-8">
          {/* Mapa (40% del ancho) */}
          <div className="w-full md:w-2/5 h-96">
            <iframe
              src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3948.092352488776!2d-62.740677625204256!3d8.293607900066982!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8dcbf97ca6fd20d7%3A0x327140684f121c8f!2sSanto%20Tome%20IV%20I!5e0!3m2!1ses!2sve!4v1738710176244!5m2!1ses!2sve"
              width="100%"
              height="100%"
              style={{ border: 0 }}
              allowFullScreen
              loading="lazy"
              referrerPolicy="no-referrer-when-downgrade"
              title="Ubicación de la empresa"
            ></iframe>
          </div>
          {/* Espacio para texto (60% del ancho) */}
          <div className="w-full md:w-3/5 flex flex-col justify-center">
            <h3 className="text-2xl font-semibold text-gray-800 mb-4">Nuestra dirección</h3>
            <p className="text-lg text-gray-600 mb-4">
              Santo Tomé IV I, Ciudad Guayana, Venezuela
            </p>
            <p className="text-lg text-gray-600 mb-4">
              Horario de atención: Lunes a Viernes de 9:00 a 18:00
            </p>
            <p className="text-lg text-gray-600">
              Estamos ubicados en una zona estratégica para brindarte el mejor servicio. ¡Visítanos!
            </p>
          </div>
        </div>
      </div>

      {/* Sección: Contacto */}
      <div className="max-w-4xl mx-auto py-12 px-4">
        <h2 className="text-3xl font-bold text-gray-800 mb-6 text-center">Contacto</h2>
        <div className="text-center">
          <p className="text-lg text-gray-600">
            Para más información, no dudes en contactarnos a través de nuestro correo electrónico:
            <a href="mailto:info@tudominio.com" className="text-blue-600 hover:underline">
              info@tudominio.com
            </a>
            . También puedes llamarnos al <strong>+58 123 456 7890</strong>. Estamos aquí para
            ayudarte.
          </p>
        </div>
      </div>
    </div>
  );
};

export default Nosotros;